...
- [x] NPC Glossary: DM-managed list of campaign NPCs in Campaign Management (section between Public Notes and Characters, only after the campaign is saved). `CampaignNpc` type on `Campaign.npcs?` (`types/campaign.ts`): name, location, details (HTML via RichTextEditor — hyperlinks preserved by DOMPurify in RichTextDisplay), imageData (upload → `processSquareImage` 300px JPEG) OR imageUrl (external link; setting one clears the other), `hidden` (default TRUE on create). DM UI in `src/components/campaigns/NpcGlossarySection.tsx`: list rows with thumb/name/location/Hidden-Visible badge, Unhide/Hide toggle, Edit, two-step Delete; modal editor. Persistence via `campaignRepository.update` + store `updateCampaign` (immediate, not part of the campaign-draft Save flow). Sharing: `CampaignContent.npcs?` (`types/sharing.ts`) — `extractPublicContent()` filters to UNHIDDEN only (hidden NPCs never leave the DM's device); `useCampaignContentSync` hash extended with `npcs: (id,updatedAt,hidden)` so NPC edits trigger the debounced push. Player view in sheet Features ▸ Notes (`NotesPanel.tsx`): collapsible "NPC Glossary" section ABOVE the notes editor (collapsed by default, count badge, ↻ Refresh), lazy-fetches on first expand via `getNpcGlossaryForCharacter(characterId)` in `sharingService.ts` (character_summaries.id === character id → one campaign per character; returns null for unlinked/offline → friendly empty state). Fetch wrapped in an 8s `Promise.race` timeout so a stalled connection degrades to the empty state instead of hanging on "Loading…". Note: `character_summaries.id` is a uuid column — non-uuid ids 400 (handled via error→null).
