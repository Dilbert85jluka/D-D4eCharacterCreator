...
│   ├── homebrew/              # HomebrewEditorModal, HomebrewImportModal, HomebrewShareCodeModal, EditorLayout, TierEditor, HomebrewBadge, 12 type-specific editors (incl. RaceEditor, ClassEditor)
...
│   ├── homebrewExport.ts       # Player-to-player homebrew sharing: buildExport, downloadExport, parseHomebrewImport, prepareImport (skip/replace/duplicate conflict modes), encodeShareCode/decodeShareCode (URL-safe base64 codes), buildShareUrl
...
│   ├── HomebrewWorkshopPage.tsx   # Homebrew Workshop — create/edit/delete custom content + per-item Share (code/URL) and Export, plus Export All / Import (player-to-player JSON + base64 share-code sharing); auto-opens import modal when arriving via `?import=<code>` deep-link
│   └── InstructionsPage.tsx       # User manual (Sidebar → Settings → User Instructions)
├── store/
│   ├── useAppStore.ts         # Navigation, sidebar, toasts, active character, pendingImportCode (captured from `?import=` URL, persisted via sessionStorage to survive magic-link redirect)
...
interface PowerData {
  ...
  actionType: 'standard' | 'move' | 'minor' | 'free' | 'immediate-interrupt' | 'immediate-reaction' | 'opportunity' | 'no-action';
...
- **Roll animation:** `DiceRollAnimation.tsx` — SVG sprite dice (classic 2D silhouettes per die type: d20 hexagon, d12 pentagon, d10/d% kite, d8 diamond, d6 rounded square, d4 triangle, d2 coin) tumble across the screen while the sound plays: launch from the left, diminishing bounce hops, decelerating whole-turn spin (always settles upright), landing spots spread evenly with jitter. Faces flicker random values while tumbling (90 ms tick) and settle on the REAL rolled values at ~80% of each sprite's flight. After settling, dice **linger fully visible for 3 s** (`DICE_LINGER_MS`) so players can read them — past the ~2.2 s result reveal — then fade out over 700 ms (`DICE_FADE_MS`, matching the overlay's `duration-700` class); the owner unmounts the overlay after the fade (`DICE_OVERLAY_CLEANUP_MS`), with a reference-equality guard so a re-roll during the linger isn't wiped by the previous roll's cleanup timer. **Single-d20 rolls outside the tray** (SkillsPanel skill checks, AbilityBlock ability checks, SheetHeader Initiative + Saving Throw) use the shared `useD20RollAnimation()` hook (`src/components/dice/useD20RollAnimation.tsx`): one amber d20 tumbles across the screen and the inline result card reveal is delayed by `DICE_SOUND_MS` (2.2 s) to match; under `prefers-reduced-motion` the hook declines (`launchD20` returns false) and the card reveals instantly as before. **In SheetHeader the overlay is rendered OUTSIDE the sticky banner div** (as a fragment sibling in both the expanded and minimized return paths) — `sticky` + `z-10` creates a stacking context that would otherwise trap the z-30 overlay inside the header's layer; keeping it outside also means an in-flight roll survives minimizing the banner. Full-screen `pointer-events-none` overlay at z-30 (under the z-40 tray). Keyframes `dice-roll-x` / `dice-roll-bounce` / `dice-roll-spin` in `src/index.css` — bounce uses per-keyframe ease-in (falls) / ease-out (rises). Max 8 sprites per roll (`MAX_SPRITES`). Skipped entirely under `prefers-reduced-motion`. Purely cosmetic — roll values are computed before the animation starts.
...
**Campaign bundle format:** `pushCampaignToCloud()` bundles the Campaign + all its CampaignSessions + all SessionEncounters + all CampaignNPCs into a single `CampaignBundle` JSONB payload (`{ campaign, sessions, encounters, npcs }`). NPCs are the **full** records (including `privateDescription`) — this is the DM's own private backup, unlike the player-facing `npc_content` push which strips DM-only fields. Bundles from before NPCs were added (no `npcs` key) pull as `npcs: []`. Push logs `[pushCampaignToCloud] "<name>" — bundling N session(s), N encounter(s), N NPC(s) from Dexie` for diagnostics.
...
- [x] Heroes of the Feywild (HotF) **class powers** — Bard + Druid: 51 bard powers (the Skald build at-wills/encounters/dailies/utilities across L1–29 plus Skald's Aura) appended to `src/data/powers/bardPowers.ts`; 43 druid powers (incl. the level-less Sentinel/companion utilities Air Spirit, Call the Spirits, Nature's Growth, Senses of the Wild, Summon Natural Ally, Verdant Touch, Vine Rope at level 0, plus L1–29 and **Meld into Stone** `druid-meld-into-stone` Utility 6) appended to `src/data/powers/druidPowers.ts`. Pulled from iws.mx `SourceBook = 'HotF'` via the Chrome-MCP live-compendium bulk-load + HTML→PowerData parser. **No HotF Warden powers exist in iws.mx** — Meld into Stone is tagged Druid-only there and was intentionally NOT duplicated under `classId: 'warden'` (it is a shared Druid/Warden Utility 6 in the printed book, but kept Druid-only to match the database). Added `'no-action'` to `PowerAction` (`character.ts`) for 9 bard weapon-rider powers (Dimensional Step / Flash of Distraction / Ringing Weapon lines); label added to the `Record<PowerAction,string>` maps in `ChannelDivinityPanel.tsx` + `DisciplinePowersPanel.tsx`; PowerCard renders it via `actionType.replace('-',' ')` + CSS capitalize. One iws.mx source typo corrected: Verdant Retaliation range `Aura burst` → `Area burst`.
...
- [x] Campaign-shared homebrew works app-wide for every joined campaign member (fixes "DM shared a race but my players can't see it"): two stacked architectural problems were both addressed.

  **Problem 1 — sources wiping each other out.** Each data type's `registerHomebrew*()` (in `src/data/races/index.ts`, `classes/index.ts`, etc.) sets `RACES = [...OFFICIAL_RACES, ...races]` — i.e. it REPLACES the homebrew section. Both `useHomebrewStore.syncToDataLayer()` (local Workshop items) and `registerCampaignHomebrew()` (DM-pushed items via realtime) called these directly. Whichever ran last won; opening a campaign view erased the user's local homebrew from the registry, and vice versa.

  **Fix:** new `src/lib/homebrewRegistry.ts` singleton holds `localItems: HomebrewItem[]` + `campaignItems: Map<sharedCampaignId, HomebrewItem[]>` and dispatches one merged register call per data type via a `dispatch()` function. Collision policy: local wins for same ID, then first-encountered campaign. Exports `setLocalHomebrew()`, `setCampaignHomebrew(scId, items)`, `removeCampaignHomebrew(scId)`, `clearAllCampaignHomebrew()`. `useHomebrewStore.syncToDataLayer` rewritten as a one-line wrapper that calls `setLocalHomebrew`. `registerCampaignHomebrew` signature changed to `(sharedCampaignId, items)` and routes through `setCampaignHomebrew`. The 13 per-type `registerHomebrew*` functions in `data/*` are unchanged — `homebrewRegistry` is the only caller now.

  **Problem 2 — players only saw DM homebrew while viewing the campaign view.** `useRealtimeCampaign` only subscribed to `shared_campaigns` UPDATE events for the actively-viewed campaign, and there was no initial pull — so a player joining a campaign whose DM had already pushed homebrew never got the items, and even after receiving them via realtime, the data was lost on page reload.

  **Fix:** new `src/hooks/useCampaignHomebrewSync.ts` runs app-wide (mounted from `App.tsx`). On login + whenever `useSharingStore.sharedCampaigns` changes, it (1) seeds each campaign's contributions from the SharedCampaign rows already in the store (`sc.homebrew_content`), and (2) opens one realtime channel per joined campaign filtering for UPDATEs on `shared_campaigns id=eq.<scId>` — when it fires, calls `setCampaignHomebrew(scId, items)`. Drops channel + registry bucket when the user leaves a campaign or logs out. Also added `loadSharedCampaigns(user.id)` to App.tsx's auth-resolved effect so the joined-campaigns list is populated on every login — previously it only loaded when the user visited the campaign management page or shared campaign view. End-to-end result: DM saves homebrew with `campaignIds: [campaign-id]` → `useHomebrewContentSync` pushes to `shared_campaigns.homebrew_content` (debounced 3s) → every player's `useCampaignHomebrewSync` realtime channel fires → `setCampaignHomebrew` updates the merged registry → the homebrew race shows up in the wizard, sheet, workshop browser, anywhere `getRaceById` is called. Survives navigation between views. On reload, the registry is re-seeded from the sharedCampaigns store load before any UI tries to read it.

- [x] User Instructions page expanded from skeleton to full user manual: rewrote `src/pages/InstructionsPage.tsx` from 8 brief sections (most 1–2 sentences) into 14 detailed sections covering every major feature. Sections now use sub-headers (`h3`) and itemized lists for scannable lookup: Getting Started (sign-in, navigation, offline support), Wizard (each of 10 steps detailed), Sheet (header + Min/Info toggles + click-to-roll + hover breakdowns), Actions & Dice Rolling (combat actions, second wind, dice FAB, implicit rolls), Powers & Class Systems (slot rules, picker, quick tray, then per-class: wizard spellbook, warlock pact, psionic augmentation, fighter combat style, monk full discipline, half-elf dilettante, CD classes), Feats (budget, prereqs, granted powers, repeatable, SIT, passive bonuses), Features/Profile/Notes, Inventory (masterwork + magic enchants on armor/weapons/implements, magic items, rituals), Rests & Leveling, Campaigns (DM create + share code + player join + roster + sync), DM Tools (sessions, encounters, initiative tracker, NPC glossary with visibility model), Compendiums (magic items 3-edition toggle + random roller; monster filters), Homebrew (13 types + sharing via file/code/URL/campaign-attached, conflict resolution), Backup/Mobile/Tips. Anchored TOC + smooth scroll preserved.
- [x] NPC glossary (Phase 2 — player sync via Supabase): the DM's visible NPCs (with `privateDescription`, `currentHp`, `maxHp`, and `level` stripped via `toPublicNPC`) auto-push to `shared_campaigns.npc_content JSONB` and stream to every connected player via the existing `useRealtimeCampaign` subscription (no new realtime channel needed — the existing `shared_campaigns` UPDATE event fires for `campaign_content`, `homebrew_content`, AND now `npc_content`). **Required SQL migration** (run once in Supabase SQL editor): `ALTER TABLE shared_campaigns ADD COLUMN IF NOT EXISTS npc_content JSONB;` — no RLS changes needed, existing policies cover the new column. New `src/lib/npcContentSync.ts` (`extractPublicNpcs`, `pushNpcContent`). New `src/hooks/useNpcContentSync.ts` — fingerprint-based per-campaign change detection (`(scId, npcId, visibility, updatedAt)`) + 3s debounce, silent fail with console warn on offline / missing column (mirrors `useHomebrewContentSync`). Wired into `App.tsx`. **Type**: `SharedCampaign.npc_content: PublicNPC[] | null` added to `src/types/sharing.ts`. **Player UI**: new `PublicNpcSection.tsx` in `SharedCampaignView` below Party Members — shows portrait, name, race/class, location, and the public description; sex/alignment/location detail grid in expanded view; auto-search when ≥4 NPCs. Read-only — no DM-only controls. Empty state when no NPCs unlocked yet: *"No NPCs unlocked yet — the DM will reveal them as the party meets them."* The toggle in the DM-side `NpcSection` (👁/🔒) now end-to-end: toggle visible → 3s debounce → push to Supabase → realtime fires → player's `PublicNpcSection` updates within seconds.
- [x] NPC glossary (Phase 1 — DM-only, local): Campaign management page got a new "Non-Player Characters (NPCs)" section between the Characters list and any session/encounter UI. Lets the DM build a glossary of NPCs scoped per campaign so long campaigns with lots of named characters stay manageable. **Fields** per NPC: name, sex, alignment (PC alignment enum), race (free-text — accepts "Mind Flayer", "Demon", etc.), class/role (free-text — "Innkeeper", "Royal Vizier"), level, current/max HP, location (home or where met), portrait (same `processSquareImage` pipeline as PC + monster portraits — 300×300 JPEG, ~25–40 KB), public description (TipTap rich text, shown to players when visible), private description (TipTap, DM-only, never synced). **Visibility model**: each NPC has a `visibleToPlayers` flag, default `false`. DM can flip 👁/🔒 inline from the list row. **Filter pills** All / Visible / Hidden + search across name / location / race / class. **Storage**: Dexie v9 — new `npcs` table indexed on `id`, `campaignId`, `name`, `updatedAt`, `visibleToPlayers`. New `npcRepository.ts` (CRUD + `deleteByCampaignId` for cascade) and `useNpcsStore.ts` (grouped by `campaignId`, alphabetically sorted). **Cascade**: `useCampaignsStore.deleteCampaign` now also calls `npcRepository.deleteByCampaignId` so orphans don't accumulate. Phase 2 (separate work, requires Supabase SQL migration): adds an `npc_content JSONB` column on `shared_campaigns`, a push hook that strips `privateDescription` + DM stats from each visible NPC, and a player-facing list in `SharedCampaignView`. **Files**: new `src/types/npc.ts`, `src/db/npcRepository.ts`, `src/store/useNpcsStore.ts`, `src/components/sharing/NpcEditorModal.tsx`, `src/components/sharing/NpcSection.tsx`; modified `src/db/database.ts`, `src/App.tsx`, `src/store/useCampaignsStore.ts`, `src/pages/CampaignManagementPage.tsx`, CLAUDE.md.
- [x] Collapsible character-sheet banner for small screens: the sticky `SheetHeader` already had a per-section "Info" toggle that hid the four info rows, but the portrait + name + 7-button action row still took ~120-140px of vertical space — half the visible height on an iPhone landscape and a quarter on portrait. Added a second toggle ("▲ Min" / "▼") that minimizes the **entire** banner down to a single ~32px strip showing only: 28×28 portrait thumbnail, character name (truncated), level badge, and an expand chevron. State persists per-character (`localStorage` key `dnd4e-sheet-header-collapsed:<id>`) so the choice survives navigation + reloads. Doesn't affect the existing "Info" toggle — they're independent: users on tablets who want most of the info visible but still want a more compact action row can use Info-collapsed-but-banner-expanded, while iPhone users can stay fully minimized and tap to expand only when they need to roll initiative, level up, etc. The minimized strip is still `sticky top-0 z-10` so the character is always identifiable while scrolling.
- [x] Magic Item + Magic Implement editors — split & multi-select enhancement targets: the old single-dropdown enhancement-type field bundled "Fortitude, Reflex, and Will" into one option and "Attack rolls and damage rolls" into another, with no way to pick a subset or combine across categories. Now both editors use a chip multi-select (`EnhancementTargetsPicker`) over six discrete targets: **AC**, **Fortitude**, **Reflex**, **Will**, **Attack rolls**, **Damage rolls**. Homebrew can grant any combination — e.g. an amulet with +1 Will *only*, or a wondrous item that grants +1 to Will *and* +1 to attack rolls. **Data model**: added `enhancementTargets?: EnhancementTarget[]` to `MagicItemData` and `MagicImplementData` (the user explicitly scoped this to those two types, not magic armor or magic weapons). Kept the legacy `enhancementType` string field for backward compat — `resolveEnhancementTargets(item)` in `gameData.ts` prefers the structured array and falls back to parsing the string ('Fortitude, Reflex, and Will' → ['fortitude', 'reflex', 'will']) so existing official amulets keep working unchanged. Editors save the new field and clear the legacy string. **Runtime**: `useCharacterDerived.ts` `magicBonuses` reducer now reads each equipped magic item's resolved targets and adds `tier.enhancement` to each selected channel — Fort/Ref/Will/AC accumulate into existing defense calcs, attack/damage accumulate into new `magicItemAttackBonus` + `magicItemDamageBonus` fields on `DerivedStats`. `CombatActionsPanel.tsx` adds these to every equipped weapon's attack roll and damage roll, with a separate "+N magic item" breakdown row. Magic weapon's own `weaponEnhancementBonus` continues to apply only to its own weapon (unchanged). Legacy fallback preserved: a neck-slot item lacking both fields defaults to all three NADs (matches existing 4e rule).
- [x] Party-roster portrait lightbox: clicking a character's avatar in the campaign party roster (anywhere `PartyRosterCards.CharacterCard` is rendered — DM campaign management page + player Shared Campaign view) opens a full-screen lightbox showing the portrait at native size. The portrait button uses `cursor-zoom-in` and the lightbox uses `cursor-zoom-out`; close via the X button (top-right), click on the dark backdrop, or press Escape (`useEffect` window keydown listener that auto-detaches when the lightbox closes). Image capped at `max-w-[min(90vw,90vh)] max-h-[90vh]` so portraits scale up but never overflow on narrow viewports. **Crucially uses `e.stopPropagation()`** on the portrait click so it doesn't also trigger the parent row's "open read-only sheet" click handler — clicking the portrait shows the picture only; clicking anywhere else on the row still opens the sheet. Cards with no portrait keep the existing "?" placeholder which is not clickable.
- [x] Read-only character sheet crash fix — clicking the portrait on another player's character no longer crashes the app: two bugs were stacked. **Bug 1**: `useCharacterDerived` accepted `Character` (not `Character | undefined`) but `useCharacterSync` passed `character as Character` even when the character was undefined. When SheetPage rendered with an `activeCharacterId` that wasn't in the local store (e.g. stale or pointing at another player's character), the hook chain crashed at `character.raceId` before SheetPage's redirect effect could fire. Fix: changed the parameter type to `Character | undefined` and added an `EMPTY_DERIVED` sentinel returned early when no character — every field zero/empty/safe. Dropped the `as Character` cast in `useCharacterSync.ts`. **Bug 2**: the portrait button in `SheetHeader.tsx` called `navigate('portrait', character.id)` unconditionally, even when rendered inside the read-only viewer modal — so clicking a foreign character's portrait set `activeCharacterId` to that character's ID and switched the view to 'portrait', dragging the user into a screen with no valid local character. Fix: gate the click handler on `!readOnly` (using the existing `useReadOnly()` context) and disable the button + drop the hover-ring styling when read-only.
- [x] Homebrew ClassEditor — Class Powers picker matches RaceEditor pattern: replaced the old single-select dropdown + add-button with the same All/Official/Homebrew filter pills + search + checkbox list UI used by the racial-powers picker in `RaceEditor.tsx`. **Eligible powers** = every homebrew power (any classId, labeled with violet "Homebrew" badge) plus every official power across all 22 classes (labeled with stone "Official" badge + a blue source-class+level pill like "Wizard L7" so the user can tell which class a borrowed power came from). Each row shows the name, source badges, usage (At-Will/Encounter/Daily), action type, and a flavor preview. **Volume guard**: ALL_POWERS contains ~1800+ entries — the list caps rendering at the first 200 matches with a "narrow your search" footer, so the picker stays responsive when "All" is selected with no search query. **Orphan-ID handling** mirrors RaceEditor — class powers whose IDs don't resolve to anything in the current registry show in a separate amber-bordered box with raw IDs + remove buttons. Selected powers go into `ClassData.classPowerIds`, which the runtime already auto-grants to characters of that class via the existing class-features pipeline. Behavior unchanged on save — only the editor UX changed.
- [x] Weapon proficiency lookup unified — fixes Runepriest Wrathful Hammer, Weapon Proficiency feats, Dwarven Weapon Training, Eladrin Soldier, multiclass weapon-proficiency choices, and paragon-path extras all silently being ignored by the attack-bonus calculation. New utility `src/utils/proficiencies.ts` exposes `getWeaponProficiencyLabels(character)` and `isProficientWithWeapon(character, weapon)`. The label builder aggregates class + feat + MC choice + paragon path + class build-choice grants. The runtime match supports three forms: exact name (e.g. "Longsword"), category match (e.g. "Military melee"), and property-class predicates ("Military hammers" matches Military Melee weapons with the `Hammer` property; same for axes, maces, picks, flails, spears, bows, crossbows, staves). `CombatActionsPanel.tsx` previously called a local `isProficientWith(weapon, cls)` that only checked `cls.weaponProficiencies` — every other source was discarded, so a Runepriest with Wrathful Hammer wielding a Warhammer was shown "⚠ Not proficient" and lost the +2 proficiency bonus. Now uses `isProficientWithWeapon(character, weapon)`. `ProficienciesPanel.tsx` rebuilt to source its weapon list from the same helper so the displayed list and the attack-bonus calc cannot drift apart. Runepriest Wrathful Hammer specifically: `character.runepriestArtistry === 'wrathful'` adds "Military hammers" and "Military maces" to the proficiency set, which the property-class predicate resolves at attack time.
- [x] Unarmored Agility feat now applies its +2 AC: `bonuses: { ac: 2, acArmorCondition: 'Cloth' }` added to the feat entry in `src/data/feats/index.ts`. Previously the feat had no `bonuses` block, so taking it had no mechanical effect. The match works because `useCharacterDerived.ts` treats unequipped armor as `'Cloth'` (`armorType = equippedArmorData?.type ?? 'Cloth'`) — so the single `acArmorCondition: 'Cloth'` covers both branches of the feat text ("cloth armor or no armor"). The +2 appears as a "Feats" row in the AC defense breakdown (rendering was already wired via the existing `rowIf(featBonuses.ac > 0, 'Feats', featBonuses.ac)` in the breakdown builder).
- [x] Homebrew Monster editor — structured power authoring: `MonsterEditor.tsx` now exposes the full 4e power structure per power entry instead of cramming everything into a single "Description" textarea. **Action dropdown** expanded from 7 to 10 user-facing options (with internal `(action, actionLabel)` mapping for backward-compat): "No Action (Trait)" for passive features like Aquatic, Standard / Move / Minor / Free, three explicit Triggered sub-types (Immediate Interrupt, Immediate Reaction, Opportunity Action — all map to `action: 'Triggered'` with an `actionLabel` override), generic "Triggered (other)", and Aura. **Structured fields** added to `MonsterPower` (all optional): `range`, `target`, `attack`, `trigger`, `hit`, `miss`, `effect`. Editor shows Range/Target/Attack/(Trigger)/Hit/Miss/Effect rows for action-type powers; Traits and Auras get a simpler Name+Description layout (no attack roll/Hit/Miss makes sense). **Usage field** got a `<datalist>` autocomplete with the canonical values (At-Will / Encounter / Daily / Recharge 6 / Recharge 5-6 / Recharge 4-6 / Recharge 3-6) while still accepting any string. **MonsterModal display** renders the structured fields as colored labeled rows (Trigger orange, Hit emerald, Miss red, Effect blue — same palette as PowerCard) above the `description` text; legacy iws.mx-imported monsters (which only set `description`) render unchanged. `actionLabel` overrides the action chip text so an Immediate Reaction shows "Immediate Reaction" instead of "Triggered". All new fields are optional — every existing monster keeps working without changes.
- [x] Draconomicon: Metallic Dragons monster source: 108 monsters from `Draco: Metallic` in iws.mx (109 primary; 1 deduped against earlier sources). New `MonsterSource` value `'dracometallic'`. Generator at `C:\Claude\generate_dracometallic.js`, output at `src/data/monsters/dracometallic.ts` with `dracometallic-` ID prefix. Wired into `OFFICIAL_MONSTERS` spread, `DEFAULT_FILTERS.sources`, and `MonsterCompendiumPage` SOURCES with label "Draco: Metallic". Verified zero name overlap with Draco: Chromatic — the two Draconomicon books cover different dragons. Quality check: ALL CLEAR.
- [x] Draconomicon: Chromatic Dragons monster source: 115 monsters added from `Draco: Chromatic` in the iws.mx listing (117 primary; 2 deduped — Adamantine Dragon and Galvanizing Dragon already existed in MM/MV). New `MonsterSource` enum value `'dracochromatic'`. Generator: `C:\Claude\generate_dracochromatic.js` (copy of `generate_mm3.js` with: `r[7] === 'Draco: Chromatic' || startsWith('Draco: Chromatic,')` filter; `source: 'dracochromatic'`; `dracochromatic-` ID prefix; output to `src/data/monsters/dracochromatic.ts`; new pre-load name dedup against all already-loaded sources to prevent reprinted dragons appearing twice). Listed in `MonsterCompendiumPage` SOURCES with label "Draco: Chromatic" and added to `DEFAULT_FILTERS.sources`. Quality check: ALL CLEAR (no truncated names, no orphan ✦ artifacts, no description-starts-with-`:` issues).
- [x] Racial powers shown in Step 10 (Review and Create): The wizard's review step previously listed only `wizard.selectedPowerIds` + the Half-Elf dilettante power under the POWERS section. Now also lists each racial power from `race.racialPowerIds` + `subrace?.racialPowerIds` (subrace lookup mirrors the existing pattern in `PowersPanel.tsx` / `ActionsByTypePanel.tsx`). Racial powers render with the standard usage badge plus a green "Race" badge so they're distinguishable from class powers, and they expand to the same `PowerDetail` panel on click. The section's outer visibility check now includes `racialPowerIds.length > 0`, so a character with only racial powers (no class powers selected yet) still shows the section. Racial powers were already listed inside the expandable race-detail card on Step 10 — this puts them in the main POWERS list where the user expects to see everything they're about to gain.
- [x] Racial powers picker — official + homebrew sources: RaceEditor's "Racial Powers" section lists both official racial powers (`ALL_POWERS.filter(p => p.classId === 'racial')`) and the user's homebrew powers (any classId) in a single combined picker. Each row shows a violet "Homebrew" or stone "Official" source badge alongside the existing usage / action-type badges. **Source filter pills** above the search input (`all` / `official` / `homebrew`) let the user narrow the list. Search filters by name across the active source. The selected IDs save into `RaceData.racialPowerIds` exactly as before — runtime resolution via `getPowerById()` is unchanged.
- [x] Racial powers picker in homebrew RaceEditor: New "Racial Powers" section in `src/components/homebrew/RaceEditor.tsx` lets the user attach their homebrew powers as racial powers for the race. Lists every homebrew power (any `classId`) with checkbox toggle, sorted alphabetically, with a search box that activates when ≥1 power exists. Selected powers populate `RaceData.racialPowerIds`, which the existing runtime (PowersPanel, ActionsByTypePanel, RacialFeaturesPanel, Step10_Review) already resolves via `getPowerById()`. Each power row shows name + usage badge (At-Will/Encounter/Daily) + action type + flavor preview. **Orphan-ID handling**: if `racialPowerIds` contains IDs that don't match any homebrew power (e.g. references to official racial powers like `fey-step`, or homebrew the user hasn't imported), they're shown in a separate amber-bordered box with raw IDs and a remove button — making the reference visible but not blocking it (resolves at runtime if found). When the user has zero homebrew powers, the empty state hint points them at the Workshop's Powers section. Replaces the previous behavior where `racialPowerIds` was preserved silently across edits but never editable in the UI.
- [x] Inline-editable appearance fields in ProfilePanel: All 7 appearance fields (Gender, Age, Height, Weight, Build, Eye Color, Hair Color) on the character sheet's Profile tab are now inline text inputs instead of read-only cards. Each input keeps a local `draft` state and commits via `characterRepository.patch()` + `useCharactersStore.updateCharacter()` on blur (or Enter); Escape reverts. Empty fields render with a placeholder (e.g. `e.g. 5'10"`) so the user knows they're editable. The card border highlights amber on focus. Read-only mode (campaign view) keeps the original filtered card grid. New `AppearanceInput` helper component scoped to `ProfilePanel.tsx`. `useEffect` keeps the local draft in sync if the upstream character changes (e.g. cloud sync push from another device).
- [x] Equipment picker alphabetical ordering: All equipment selection lists in both the wizard (`Step8_Equipment.tsx` — Weapons / Armor / Gear tabs) and the character sheet (`EquipmentPanel.tsx` — Weapons / Armor / Magic Armor tiers / Magic Weapon tiers / Implements / Superior Implements / Magic Implement tiers / Magic Items / Consumables / Gear) now sort by name (case-insensitive `localeCompare`) after the search filter is applied. For tier-flattened entries (magic armor/weapon/implement), the secondary sort is `tier.level` ascending so the cheapest tier of a given item appears first. Owned-inventory listings still group by slot/equipped-status as before — only the **add-equipment pickers** changed. Generic `byName` helper at the top of `EquipmentPanel.tsx`'s filter block to keep the per-list sorts terse.
- [x] Homebrew share-link auto-handler: Deep-link `?import=<code>` URLs trigger the import flow automatically. **App.tsx** captures the URL parameter on first paint, stashes the code in `useAppStore.pendingImportCode`, and strips it from the URL via `history.replaceState` so a refresh doesn't repeat. After auth resolves, App.tsx navigates to the Homebrew Workshop. **HomebrewWorkshopPage** reads `pendingImportCode`, opens the import modal pre-populated with the code, then clears the pending state. **HomebrewImportModal** accepts an `initialCode` prop — when supplied it defaults to the "From code" tab and auto-decodes once via a `useRef` guard, so the user lands directly on the preview + conflict-resolution UI. Code survives the login redirect (sessionStorage-backed Zustand persist on `useAppStore` keeps `pendingImportCode` across the magic-link round-trip). End-to-end flow: someone shares a `https://dnd4ebuilder.com/?import=eyJ0eXBl...` link → recipient clicks → app loads → (if needed, recipient signs in) → import preview opens automatically.
- [x] Homebrew share code / share URL: Second sharing mechanism alongside JSON file export. Each row in `HomebrewWorkshopPage` has a "Share" button that opens `HomebrewShareCodeModal` (`src/components/homebrew/`) — shows a copyable URL-safe base64 share code AND a deep-link share URL (`<origin>/<path>?import=<code>`). Recipient pastes either form into the import modal's new "From code" tab and clicks Decode; same preview + skip/replace/duplicate conflict resolution as the file path. **Encoding** (`src/lib/homebrewExport.ts`): `encodeShareCode(items)` wraps the same export envelope used by the file path, `JSON.stringify`s it, then URL-safe base64s (RFC 4648 §5: `+`→`-`, `/`→`_`, strip `=` padding) for inline use. `decodeShareCode(input)` accepts either a bare code or a full URL and re-uses `parseHomebrewImport()` for envelope validation — same error-handling pipeline as the file path. **Round-tripped via `encodeURIComponent`/`unescape`** so non-Latin-1 characters survive `btoa`/`atob`. **HomebrewImportModal** got a tabbed UI: "From file" (file picker, original behavior) / "From code" (textarea + Decode button). Modal accepts an optional `initialCode` prop so a future `?import=<code>` URL handler can pre-populate the code tab. **Size warning** in the share modal when the URL exceeds ~8 KB (some chat apps truncate); falls back to suggesting users send the code only. Codes for typical homebrew items run 1–3 KB; an entire homebrew class can hit ~5 KB. Uncompressed — kept simple to avoid pulling in pako/CompressionStream.
...
- [x] NPC cross-device sync + empty-push clobber fix (fixes "NPCs disappear when the DM opens the app on the iPad"): two stacked bugs. **Bug 1** — NPCs were never part of the cross-device cloud backup: `CampaignBundle` only carried `{ campaign, sessions, encounters }`, so a DM's second device pulled the campaign but zero NPCs. **Bug 2** — `useNpcContentSync` on that second device then computed its visible-NPC list from the empty local store and pushed `[]` to `shared_campaigns.npc_content`, wiping the players' NPC list until the first device re-pushed (flip-flop between devices). **Fix**: (1) `CampaignBundle` gains `npcs: CampaignNPC[]` — `pushCampaignToCloud()` bundles full NPC records from Dexie, `pullAllCampaignsFromCloud()` defaults missing `npcs` to `[]` for legacy rows, `mergeCloudCampaigns()` upserts NPCs per-record newer-wins by `updatedAt` and force-reloads `useNpcsStore`. (2) `useCampaignCloudSync` fingerprints include each NPC's `updatedAt` (watching `npcsByCampaign`) so NPC edits trigger a bundle push; baseline capture waits for the NPC store to load. (3) New `cloudPullDone` flag on `useCampaignsStore`, set by `useCampaignCloudSync` after the startup pull attempt finishes; `useNpcContentSync` refuses to push until both `useNpcsStore.hasLoaded` AND `cloudPullDone` are true — a cold device can no longer blank out `npc_content`. No Supabase schema change needed (NPCs ride inside the existing `campaign_data` JSONB). The additive-only-merge limitation this feature originally shipped with (deleted records resurrecting from another device's push) was fixed by the tombstone-delete system below.

- [x] Tombstone-style deletes for sessions, encounters, and NPCs (fixes "a record deleted on one device resurrects from another device's later push"): the per-record newer-wins cloud merge was additive-only — it had no way to distinguish "device B never had this record" from "device B deleted it", so the live copy always won. **Design**: deleting a session/encounter/NPC no longer removes the Dexie record; it becomes a **tombstone** — `deleted: true`, `updatedAt` bumped to now, heavy content blanked (session rich-text fields; encounter description/monsterEntries/characterIds/initiativeState; NPC portrait + both descriptions). A tombstone is just "the newest version of the record", so the existing per-record newer-wins merge propagates deletions with zero changes to the merge logic itself. **Reads**: all repository query methods (`getAll`, `getAllForCampaign`, `getAllForSession`, `getByCampaignId`) filter `!deleted` — UI, stores, fingerprints, and the player-facing `npc_content` push never see tombstones. **Push**: `pushCampaignToCloud()` uses new `*IncludingDeleted` repo variants so tombstones ride in the bundle (logs tombstone count). **Cascades**: deleting a *session* tombstones its encounters (so they don't outlive their tombstoned parent); deleting a *campaign* still hard-deletes all children — no tombstones needed since the whole cloud row is soft-deleted (`user_campaigns.deleted = true`). **TTL**: `purgeExpiredTombstones()` in `src/db/tombstones.ts` hard-deletes tombstones older than 60 days, fired once from App.tsx on startup. A device offline longer than 60 days may resurrect a deleted record on its next push — accepted trade-off to bound table/bundle growth. **No Dexie schema bump** — `deleted` is an unindexed optional field, filtered in JS. The campaign-level resurrection hole that remained after this feature was fixed separately (next entry).

- [x] Session notes showed raw `<p>` tags in the player campaign view: session fields became TipTap rich text when the DM-side editors were added, but `SessionCard` in `SharedCampaignView.tsx` still rendered `importantEvents` / `additionalNotes` as plain text in a `whitespace-pre-wrap` `<p>`, so players saw the literal HTML markup. Campaign description and public notes were already going through `RichTextDisplay` — sessions were the only two renders left behind. **Fix**: both now use `RichTextDisplay`. **`RichTextDisplay` hardened for the mixed-content reality** (fields hold TipTap HTML *or* legacy plain text depending on when they were written, and both live in the same column): (1) new exported `looksLikeHtml()` — requires a real opening tag `<name …>` so prose containing a bare `<` ("damage < 10", "a<b") is correctly treated as plain text; non-HTML content takes a `whitespace-pre-wrap` path so **legacy notes keep their line breaks** instead of collapsing into one run-on paragraph, which a naive swap to HTML rendering would have silently done to every pre-editor note in the app; (2) new exported `hasRichTextContent()` — TipTap saves an emptied field as `<p></p>` / `<p><br></p>`, which is truthy as a string, so callers deciding whether to show a section header or expand affordance must use this rather than `!!value` (images/rules/tables count as content even with no text). `SessionCard` uses it for its expand chevron and per-field headers, so an emptied session no longer advertises content and then renders "No content." twice. **No data migration** — the fix is purely presentational; the HTML already in `shared_campaigns.campaign_content` renders correctly as-is.

- [x] Campaign-level delete resurrection fix (closes the last resurrection hole): deleting a campaign on device A soft-deletes the cloud row (`user_campaigns.deleted = true`), but device B still had the campaign locally and any later edit re-upserted the row with `deleted: false` — resurrecting the campaign everywhere. Fixed with last-write-wins at the **bundle level**, where "local activity" = max `updatedAt` across the campaign's own fields + all its sessions/encounters/NPCs (campaign fields alone aren't enough because child edits don't bump `campaign.updatedAt`). **Pull side**: `pullAllCampaignsFromCloud()` now returns `{ bundles, deletions }` — two queries so soft-deleted rows only ship `id + updated_at` (not their stale `campaign_data` JSONB) as `CloudCampaignDeletion` markers. `mergeCloudCampaigns(bundles, deletions)` applies deletions before merging: if the cloud deletion timestamp ≥ all local activity, the local campaign + sessions + encounters + NPCs are hard-deleted (logs `Removed local campaign "<name>" — deleted in cloud on another device`); if local activity is newer, the local copy is kept and the next push deliberately resurrects it. **Push side**: `pushCampaignToCloud()` does a pre-push check of the cloud row's `deleted` + `updated_at`; if deleted and newer than all local activity it returns `'skipped-deleted'` without upserting (covers a device that's already running and won't re-pull until next startup). If the pre-push check itself fails (transient network), the push proceeds as before. Deletion markers stay in Supabase indefinitely (cheap: id + timestamp) so any device that's been offline arbitrarily long still receives the deletion on its next startup pull.
...
- Launches the shared d20 tumble animation via `useD20RollAnimation()`; the result card reveal is delayed by `DICE_SOUND_MS` so the card can't announce the number while the die is still flickering. Instant reveal under `prefers-reduced-motion`.
...
- **Always calls `playDiceRollSound(1)`** when clicked
- Launches the shared d20 tumble animation via `useD20RollAnimation()`, with the same `DICE_SOUND_MS`-delayed reveal as Initiative
- Clears `initResult` on click so the two result boxes never overlap
- **Delayed-reveal race guard:** because Initiative and Saving Throw clear each other AND the reveal is now delayed, a pending reveal can outlive the roll that scheduled it — rolling Initiative then Saving Throw 1 s later would pop BOTH cards open. Every roll takes a token from a shared `rollTokenRef`; a reveal only applies if its token is still the newest. Do not remove this guard when touching either handler.
...
| `Draco: Chromatic` | Draconomicon: Chromatic Dragons |
| `Draco: Metallic` | Draconomicon: Metallic Dragons |
...
| `Draco: Chromatic` | Draconomicon: Chromatic Dragons | 115 (117 primary in listing, 2 deduped against MM/MV) |
| `Draco: Metallic` | Draconomicon: Metallic Dragons | 108 (109 primary in listing, 1 deduped against earlier sources) |
...
| `C:\Claude\generate_dracochromatic.js` | `src/data/monsters/dracochromatic.ts` | 115 |
| `C:\Claude\generate_dracometallic.js` | `src/data/monsters/dracometallic.ts` | 108 |
...
```typescript
export type MonsterSource = 'mm1' | 'mm2' | 'mm3' | 'dmg' | 'dmg2' | 'mv' | 'mvttnv' | 'dracochromatic' | 'dracometallic';
...
interface MonsterPower {
  name: string;
  /** Bucket — 'Trait' = passive feature (no action); 'Aura' = passive emanation; 'Triggered' covers Immediate Interrupt/Reaction/Opportunity (use actionLabel to disambiguate). */
  action: 'Standard' | 'Move' | 'Minor' | 'Free' | 'Triggered' | 'Trait' | 'Aura';
  /** Display override — e.g. 'Immediate Interrupt', 'Immediate Reaction', 'Opportunity Action'. MonsterModal renders this in the action chip when present. */
  actionLabel?: string;
  keywords?: string[];
  recharge?: string;   // e.g. 'Encounter', 'Daily', 'Recharge 5-6', '2/Encounter'
  // ── Structured power components (optional). When any are present, MonsterModal renders them as labeled rows before description.
  range?: string;     // 'Melee 1', 'Ranged 10', 'Close burst 2', 'Close blast 3', 'Area burst 1 within 10'
  target?: string;    // 'One creature', 'Each enemy in burst'
  attack?: string;    // '+10 vs AC', '+8 vs Reflex'
  trigger?: string;   // For Triggered/Immediate actions
  hit?: string;
  miss?: string;
  effect?: string;
  description: string; // Free-form catch-all. Required for backward compat (existing iws.mx-imported powers store everything here).
}
```
