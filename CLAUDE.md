...
- **Roll animation:** `DiceRollAnimation.tsx` — SVG sprite dice (classic 2D silhouettes per die type: d20 hexagon, d12 pentagon, d10/d% kite, d8 diamond, d6 rounded square, d4 triangle, d2 coin) tumble across the screen while the sound plays: launch from the left, diminishing bounce hops, decelerating whole-turn spin (always settles upright), landing spots spread evenly with jitter. Faces flicker random values while tumbling (90 ms tick) and settle on the REAL rolled values at ~80% of each sprite's flight. After settling, dice **linger fully visible for 3 s** (`DICE_LINGER_MS`) so players can read them — past the ~2.2 s result reveal — then fade out over 700 ms (`DICE_FADE_MS`, matching the overlay's `duration-700` class); the tray unmounts the overlay after the fade (`SPRITE_CLEANUP_MS`), with a reference-equality guard so a re-roll during the linger isn't wiped by the previous roll's cleanup timer. Full-screen `pointer-events-none` overlay at z-30 (under the z-40 tray). Keyframes `dice-roll-x` / `dice-roll-bounce` / `dice-roll-spin` in `src/index.css` — bounce uses per-keyframe ease-in (falls) / ease-out (rises). Max 8 sprites per roll (`MAX_SPRITES`). Skipped entirely under `prefers-reduced-motion`. Purely cosmetic — roll values are computed before the animation starts.
